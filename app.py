...
# FIX 1: Robust ATM strike selection — find strike closest to spot
strikes_list = df['strike'].tolist()
atm_strike   = min(strikes_list, key=lambda x: abs(x - spot))  # always valid

strike = st.select_slider(
    "Strike", options=strikes_list,
    value=atm_strike  # defaults to ATM, not index-based lookup
)

# ── BSM Price + Greeks ───────────────────────────────────
row = df[df['strike'] == strike].iloc[0]

# FIX 2: Better mid-price fallback chain
...
if bid > 0 and ask > 0:
    ...
    mid_price = 0.01  # prevent zero-price IV solver crash

# FIX 3: IV — use market IV if sane, else solve, else fallback to 25%
...
# Show IV and its source so user knows what's being used
...
# FIX 4: Warn user when deep ITM/OTM (explains near-zero Gamma/Vega)
moneyness = spot / strike if opt_type == 'call' else strike / spot
if moneyness > 1.15:
    st.info("ℹ️ Deep ITM: Delta ≈ 1, Gamma/Vega ≈ 0 is mathematically correct here.")
elif moneyness < 0.85:
    st.info("ℹ️ Deep OTM: Delta ≈ 0, Gamma/Vega ≈ 0 is mathematically correct here.")
